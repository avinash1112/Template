...
generate_php_env() {
  ...
  generate_service_env_file "backend" "php" "PHP_CONFIG_FPM_PING_PATH" "PHP_CONFIG_FPM_STATUS_PATH" "MYSQL_MASTER_HOST_NAME" "MYSQL_REPLICA_HOST_NAME" "REDIS_MASTER_HOST_NAME" "REDIS_REPLICA_HOST_NAME"
  
  log_debug "PHP environment file generation completed"
}

inject_php_app_replicas() {
  local docker_compose_file="${1}"
  log_info "Injecting PHP app replica configurations"
  # TODO Implementation similar to MySQL but for PHP app
...
inject_php_workers_replicas() {
  ...
  # TODO Implementation similar to MySQL but for PHP Workers
